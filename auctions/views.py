...
def send_verification_email(email, verification_token):
    ...
    from_email = "bhukyasharath1729@gmail.com"  # Replace with your email
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            # Check if the user is registered but not verified
            try:
                ...
                if not user_profile.email_verified:
                    ...
                    user_profile.save()
                    send_verification_email(user.email, user_profile.verification_token)

                    return render(request, "auctions/login.html", {
                        "message": "A verification email has been sent. Please verify your email before logging in."
                    })
            except UserProfile.DoesNotExist:
                pass  # User does not have a user profile, proceed with login
...
def create(request):
    if request.method == "POST":
        ...
        m.bid_end_time=request.POST['hourleft']
...
def user_profile(request, username):
    ...
    user_auctions = auctionlist.objects.filter(user=request.user)
    user_bids = bids.objects.filter(user=request.user)
    user_comments = comments.objects.filter(user=request.user)
    user_watchlist = watchlist.objects.filter(user=request.user)
    user_wins = winner.objects.filter(user=request.user)

    user_bidding_items = auctionlist.objects.filter(user=request.user)
    ...
    context = {
        ...
        'user_wins': user_wins,
        'user_bidding_items': user_bidding_items,
        'user_watchlist_items': user_watchlist_items,  # Added for watchlist items
    }
